...
public class DisplayUI extends UserInterface {
	...
	public void GenerateDisplay(String display) throws IOException {
		...
		switch (display) {
			...
			case "Store": {
				FrameDis = new JFrame(display);
				ScreenDis = (DisplayUI)new StoreScreen();
				PanDis = ((StoreScreen)ScreenDis).getPanel1();
				break;
			}
			case "CollectionManger": {
				FrameDis = new JFrame(display);
				ScreenDis = (DisplayUI)new CollectionManagerSC();
				PanDis = ((CollectionManagerSC)ScreenDis).getPanel1();
				break;
			}
			default:
